...
function Get-InstalledDriverInfo {
    ...
    try {
        # Usar Get-CimInstance (compatible con PowerShell 5.1+ y 7+)
        $drivers = Get-CimInstance -ClassName Win32_PnPSignedDriver -ErrorAction Stop | Select-Object -Property `
            DeviceName,
            DriverVersion,
            DriverDate,
            Manufacturer,
        ...
        Write-Log "Se obtuvieron $($drivers.Count) drivers instalados" -Level Debug
        return $drivers
    }
    catch {
        Write-Log "Error al obtener información de drivers: $_" -Level Error
        return @()
    }
}
