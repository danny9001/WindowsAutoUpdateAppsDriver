...
.NOTES
  v8.1 - Combinacion de UpdateAppsDrivers + Reinstalar-WinGet
  ...
  - Manejo de errores mejorado para WinGet y VC++ Redistributable
#>
...
function Install-WinGet {
    ...
    if (-not (Install-VCRedist)) {
        Write-ErrMsg "La instalacion de VC++ Redistributable fallo. Abortando instalacion de WinGet."
        return $false
    }
    ...
    try {
        ...
        Invoke-Download -Uri 'https://github.com/microsoft/microsoft-ui-xaml/releases/download/v2.8.6/Microsoft.UI.Xaml.2.8.x64.appx' -OutFile $uixamlPath
        ...
        if (-not $wingetUrl -or -not $licenseUrl) {
            throw "No se pudieron encontrar los URLs de descarga para WinGet en la ultima release de GitHub."
        }

        $wingetPath = Join-Path $tempFolder 'WinGet.msixbundle'
        $licensePath = Join-Path $tempFolder 'License.xml'

        Invoke-Download -Uri $wingetUrl -OutFile $wingetPath
        ...
        try {
            ...
            throw "Fallo la instalacion de VCLibs: $($_.Exception.Message)"
        }
        ...
        try {
            ...
            throw "Fallo la instalacion de UI.Xaml: $($_.Exception.Message)"
        }
        ...
        try {
            ...
            throw "Fallo la instalacion de WinGet: $($_.Exception.Message)"
        }
        ...
        if ($wingetPackage) {
            ...
            if ($currentPath -notlike "*$wingetDest*") {
                ...
                Write-Info "WinGet agregado al PATH de la maquina. Se requiere reiniciar la sesion de PowerShell."
            }
        ...
        if (Test-WinGetInstalled) {
            Write-Ok "WinGet instalado y verificado correctamente."
            return $true
        }

        Write-ErrMsg "No se pudo verificar la instalacion de WinGet. Puede que necesite reiniciar el terminal."
        return $false

    } catch {
        Write-ErrMsg "Error critico instalando WinGet: $($_.Exception.Message)"
        return $false
    } finally {
...
function Download-RZGet {
    ...
    try {
        Invoke-Download -Uri $Global:RZGetLatestUrl -OutFile $dest
        Write-Ok "RZGet.exe descargado."
        return $dest
    } catch {
        Write-ErrMsg "Error descargando RZGet.exe: $($_.Exception.Message)"
        return $null
    }
}
...
function Update-RuckZuckApps {
    ...
    if (-not $rzPath -or -not (Test-Path $rzPath)) {
        Write-ErrMsg "No se pudo obtener RZGet.exe. Omitiendo RZGet."
        return
    }
...
#region MAIN
Write-Info "========================================"
Write-Info "  ACTUALIZADOR DE APPS Y SISTEMA v8.1"
Write-Info "  (Manejo de errores mejorado)"
Write-Info "========================================"
Write-Host ""
...
# BQcwAYYYaHR0cDovL29jc3AuZGlnaicenrt.comMEMGCCsGAQUFBzAChjdodHRw
# Oi8vY2FjZXJ0cy5kaWdpY2VydC5jb20vRGlnaUNlcnRBc3N1cmVkSURSb290Q0Eu
# Y3J0MEUGA1UdHwQ+MDwwOqA4oDaGNGh0dHA6Ly9jcmwzLmRpZ2ljZXJ0LmNvbS9E
# aWdpQ2VydEFzc3VyZWRJRFJvb3RDQS5jcmwwEQYDVR0gBAowCDAGBgRVHSAAMA0G
# CSqGSIb3DQEBCwUAA4IBAQBwoL9DXFXnOF+go3QbPbYW1/e/Vwe9mqyhhyzshV6p
...
# YW1waW5nIFJTQTEwOTYgU0hBMjU2IDIwMjUgQ0ExMIICIjANBgkqhkiG9w0BAQEF
...
# c3RlZFJvb3RHNC5jcnQwQwYDVR0fBDwwOjA4oDagNIYyaHR0cDovL2NybDMuZGlz
# aWNlcnQuY29tL0RpZ2lDZXJ0VHJ1c3RlZFJvb3RHNC5jcnwwIAYDVR0gBBkwFzAI
...
# oFKgUIZOaHR0cDovL2NybDMuZGlnaWNlcnQuY29tL0RpZ2lDZXJ0VHJ1c3RlZEdN
